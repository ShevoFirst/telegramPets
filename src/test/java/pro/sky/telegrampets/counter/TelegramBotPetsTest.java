...
class TelegramBotPetsTest {
    ...
    @Mock
    private FakeTelegramBotPets fakeTelegramBotPets;

    @Mock
    TelegramBotPets telegramBotPetsMock;
    ...
    void testExecuteSendMessage() {
        SendMessage sendMessage = new SendMessage();
        sendMessage.setChatId(123L);
        sendMessage.setText("qwe");

        telegramBot.executeSendMessage(sendMessage);
        verify(telegramBot, times(1)).executeSendMessage(messageArgumentCaptor.capture());
    ...
    void testSendImageFromFileId() {
        SendPhoto sendPhoto = new SendPhoto();
        sendPhoto.setChatId("123");
        sendPhoto.setPhoto(new InputFile("123321"));

        telegramBot.sendImageFromFileId("123321", "123");
        verify(telegramBot, times(1)).sendImageFromFileId(photoIdByReport.capture(), chatId.capture());
        ...
        assertDoesNotThrow(() -> telegramBotPetsMock.executeSendMessage(any(SendMessage.class)));
    }
    ...
    void TestReportSubmitted() {
        Message message = new Message();
        message.setText("Отчет сдан");
        Update update = new Update();
        update.setMessage(message);

        telegramBotPetsMock.reportSubmitted(update);
        verify(telegramBotPetsMock, times(1)).reportSubmitted(messageText.capture());
    ...
    void TestReportNotSubmitted() {
        ...
        telegramBotPetsMock.reportNotSubmitted(update);
        verify(telegramBotPetsMock, times(1)).reportNotSubmitted(reportNotSubmittedCaptor.capture());
    ...
    @Test
    @DisplayName("Отправляем текст непроверенного отчета")
    void reviewListOfReports() {
        telegramBotPetsMock.reviewListOfReports(123L);

        verify(telegramBotPetsMock, times(1)).reviewListOfReports(reviewListOfReports.capture());
    ...
    @Captor
    private ArgumentCaptor<SendMessage> sendButtonVolonterMessageCaptor;

    @Test
    @DisplayName("Вызов волонтеской панели")
    void sendButtonVolonter() {
        SendMessage sendMessage = new SendMessage();
        sendMessage.setChatId(123L);
        sendMessage.setText("Волонтерская панель");

        telegramBotPetsMock.sendButtonVolonter(123L);
        verify(telegramBotPetsMock, times(1)).sendButtonVolonter(sendButtonVolonterCaptor.capture());

        var volonterCaptorValue = sendButtonVolonterCaptor.getValue();
    ...
    @Test
    @DisplayName("Проверка вызова панели волонтера")
    void testSendButtonVolonter() throws TelegramApiException {
        long chatId = 123; //
        telegramBot.sendButtonVolonter(chatId);
